...
当前为第四次迭代：保持粗分区归属，联合几何代价和当前节点能力，以每节点16个分区为整组做瓶颈配对。已有第三次数据用于同规模训练（-1/17），直接评价留出的41分区；不再单独追加校准轮次。算法与离线结果见 [第四次归档](../docs/experiments/20260908_iteration4.md)。离线预测显示收益集中在4096/8192进程，实际核心时间收益尚待集群四组对照验证。
