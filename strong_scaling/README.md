...
第二轮 v2 校准已经完成；尾部预测不足的分析见 [校准结果](../docs/experiments/20260907_calibration.md)。当前 v3 迭代的模型、采用判据、验证及配置见 [完整归档](../docs/experiments/20260907_iteration_v3.md)，尚未在集群运行。
...
本次先重新构建，保持配置区的 v3 校准默认值：

```bash
EXPERIMENT_STAGE="${EXPERIMENT_STAGE:-calibration}"
```

完成校准后，在同一个配置区改两处（目录填本次 v3 新结果；此前 v2 数据缺少6个新增特征及多种子样本）：
...
两步均为每节点16进程、`MAXH=1000`、`MINH=0`。-1保留原 METIS 默认分区；17/41提供不同分区样本。结果按种子分别汇总，不混算成同一工作量的重复。成功后继续自动清理过程网格、无损压缩画像与日志。pilot 会逐种子进行稀疏面核对；其 L/R 不同，不能用于 production 模型训练。

评价前自动训练每个目标规模的冻结模型，完全排除该目标规模的校准样本；例如8192仅使用1024/2048/4096。内部再分别留出整个规模、整个种子，检查误差和最慢1%召回；特征范围和经验误差包络也只使用训练数据。先在 `models/MODEL_REPORT.md` 查看内部验证，实际性能结论仍由目标规模对照给出。模型绑定输入、二进制、L/R、线程与网格参数，不允许混用；旧八特征样本会在提交评价作业前被拒绝。
...
配置区还可以直接修改：`ALGORITHMS`（算法列表）、`TIMING_MODES`（计时模式）、`PARTITION_SEEDS`（分区种子）、`REPEATS`（正式次数）、`WARMUPS`（预热次数）、`SEARCH_SECONDS`（搜索软预算）、`BALANCE_SWEEPS`（修正轮数）、`CUT_GROWTH`（切分面增长上限）、`COST_WEIGHTS`（代理权重）、启动延时、超时和 `DRY_RUN`（仅打印提交命令）。
...
1. `RESULT_SUMMARY.txt`：第一入口；显示实验是否完整、各分区种子的有效重复次数和主要指标，评价时还显示采用比例、跳过搜索次数和拒绝原因计数。
...
- `model_samples.csv.gz`（第二轮紧凑样本）：14个阶段特征、耗时、种子、节点名和可复现身份；供训练/验证使用，不必逐行阅读。节点名仅用于残差归因，不进入耗时模型。上传结果时保留此文件；评价批次也保留根目录的 `models/`。不需要上传所有逐次原始画像。
