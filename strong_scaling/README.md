...
第四次四组评价已经完成：160次正式运行成功，稀疏通信收益得到再次验证，但节点组映射没有兑现预测中的显著净收益。8192进程组合相对仅稀疏自然核心仅下降1.44%，屏障拆分模式略慢。原因与后续主线见 [第四次实验结果](../docs/experiments/20260908_iteration4_results.md)。当前脚本配置保留用于复现本轮，不能据此将新增均衡视为已验证成功；下一轮应针对真实慢分区和调度粒度，而非继续增加校准轮次。

## 四组消融

| ALGORITHMS（算法列表）取值 | 节点组映射 | 面交换 |
|---|---|---|
...
本次先运行 `bash build_project.sh` 重新构建，再直接执行统一入口。生产配置默认 `EXPERIMENT_STAGE=evaluation`、`BALANCE_METHOD=node_mapping`；`CALIBRATION_ROOT` 已指向仓库上传的 `mesh_algorithms_20260908-135236`。不用再先做一轮大规模采样。

| 设置 | 本轮生产评价 |
|---|---|
| 训练 | 每规模使用已有种子 -1、17；登录节点自动拟合 |
| 正式分区 | 留出的41，不参与拟合 |
| 算法 | baseline、balance、sparse、combined |
| 计时 | natural、split |
| 每组预热/正式 | 1/5 |
| 全部正式/常规预热次数 | 160/32 |
| 当前节点参考预热 | 每规模1次，共4次，包含在原作业内 |
| 分区预检/作业数 | 各4次/4个并行作业 |

每节点16进程、`MAXH=1000`、`MINH=0`、单线程。预检确认41和参考 -1 的真实粗单元归属；正式运行逐单元复核。当前节点能力从 -1/sparse/natural 一次参考预热估计，随后四组共用冻结能力文件。同一重复的初始节点轮换一致，均衡仅重新分配整组分区，不运行旧边界移动搜索。预测收益不足0.35秒或5%时保留原映射。

模型使用同规模已有样本和不变的14维几何特征；明确允许历史校准程序到本次重编译程序的转移，前提是特征契约、输入、参数和参考粗划分一致。本次程序、模型、容量表仍分别记录并绑定。旧 `BALANCE_METHOD=boundary` 评价继续使用原跨规模留出和二进制绑定协议，本轮不能宣称已解决跨规模泛化。

参考预热的准备墙钟时间单列，不混入正式核心时间；映射本身计入核心时间。单次应用是否划算，还需把不能复用的准备成本加回端到端时间。文件与结果解释见第四次归档。
